...
def is_expected_starting_gkp(p: Dict[str, Any]) -> bool:
    """
    Returns True if a goalkeeper is expected to start for their Premier League club.
    Checks simulation start probability (>= 50.0%) and match playing minutes.
    """
    if p.get('position') != 'GKP':
        return False
    start_prob = float(p.get('start_prob', 0.0))
    if start_prob >= 50.0:
        return True
    if p.get('minutes', 0) >= 180 and start_prob >= 25.0:
        return True
    return False
...
def is_valid_gkp_pair(g1: Dict[str, Any], g2: Dict[str, Any]) -> bool:
    """
    Enforces FPL goalkeeper pairing rules and strategies:
    1. Mandatory: At least 1 goalkeeper MUST be expected to start (start_prob >= 50.0%).
       Prevents squads from having two non-playing £4.0m reserve keepers.
    2. Pricing & Structure Strategies:
       - Premium Starter (> £5.0m, e.g. Raya) paired with a <= £4.0m reserve backup (deadspot).
       - Rotating Budget Keepers (both <= £5.0m, with at least one <= £4.5m).
    Never permits two expensive keepers without rotation value (e.g. £5.0m + £5.0m or £6.0m + £5.0m).
    """
    # Mandatory rule: At least 1 goalkeeper must be expected to start
    if not (is_expected_starting_gkp(g1) or is_expected_starting_gkp(g2)):
        return False
...
def pick_best_starting_xi(squad: List[Dict[str, Any]], score_key: str = 'xp') -> Dict[str, Any]:
    """
    Given a 15-player squad (2 GKP, 5 DEF, 5 MID, 3 FWD),
    finds the valid starting XI formation that maximizes score_key (e.g. 'xp' or 'selected_by_percent'),
    and assigns an optimal bench order.
    """
    gkps = sorted([p for p in squad if p['position'] == 'GKP'], key=lambda x: x.get(score_key, 0), reverse=True)
    defs = sorted([p for p in squad if p['position'] == 'DEF'], key=lambda x: x.get(score_key, 0), reverse=True)
    mids = sorted([p for p in squad if p['position'] == 'MID'], key=lambda x: x.get(score_key, 0), reverse=True)
    fwds = sorted([p for p in squad if p['position'] == 'FWD'], key=lambda x: x.get(score_key, 0), reverse=True)
    ...
    for n_gkp, n_def, n_mid, n_fwd in VALID_FORMATIONS:
        ...
        total_score = sum(p.get(score_key, 0.0) for p in starters)
        if total_score > best_score:
            best_score = total_score
            best_formation = f'{n_def}-{n_mid}-{n_fwd}'
            best_starters = starters
            
            # Bench: backup keeper + remaining outfield players sorted by score_key
            bench_gk = gkps[n_gkp:]
    ...
    # If ownership/template: consensus captain is highest-owned high-upside attacker (e.g. Haaland / Pedro)
    if score_key == 'selected_by_percent':
        captain_candidates = sorted(
            best_starters,
            key=lambda x: (
                x.get('selected_by_percent', 0.0) >= 70.0 and x['position'] in ('FWD', 'MID'),
                x.get('xp', 0.0),
                x.get('selected_by_percent', 0.0)
            ),
            reverse=True
        )
    else:
    ...
    if len(gkps) >= 2:
        ...
        starter = g1 if g1.get('xp', 0) >= g2.get('xp', 0) else g2
        backup = g2 if starter['id'] == g1['id'] else g1

        if min_p <= 4.0 or not (is_expected_starting_gkp(g1) and is_expected_starting_gkp(g2)):
            strat_name = 'Set-and-Forget Premium' if max_p > 5.0 else 'Set-and-Forget Budget'
            gkp_info = {
                'strategy': strat_name,
                'premium_starter': starter['name'],
                'deadspot_backup': backup['name'],
                'schedule': []
            }
        elif max_p <= 5.0 and min_p <= 4.5 and is_expected_starting_gkp(g1) and is_expected_starting_gkp(g2):
            gkp_info = {
                'strategy': 'Rotating Budget Keepers',
                'schedule': get_gkp_rotation_schedule(g1, g2)
            }

    actual_starting_xp = sum(p.get('xp', 0.0) for p in best_starters)
    starting_ownership = sum(p.get('selected_by_percent', 0.0) for p in best_starters)

    return {
        'formation': best_formation,
        'starting_xp': round(actual_starting_xp, 2),
        'starting_ownership': round(starting_ownership, 1),
...
def solve_optimal_squad(
    ...
    score_key = 'selected_by_percent' if objective in ('ownership', 'template') else ('ceiling' if objective == 'ceiling' else ('ppm' if objective == 'value' else 'xp'))
    ...
    xi_analysis = pick_best_starting_xi(current_squad, score_key=score_key)
    total_squad_xp = sum(p.get('xp', 0.0) for p in current_squad)
    total_squad_ownership = sum(p.get('selected_by_percent', 0.0) for p in current_squad)
    ...
    return {
        ...
        'total_squad_ownership': round(total_squad_ownership, 1),
...
def generate_template_squad(
    base_dir: str = ".",
    budget: float = 100.0,
    max_per_team: int = 3
) -> Dict[str, Any]:
    """
    Solves for the consensus 15-player 'template' squad that maximizes cumulative
    FPL ownership (selected_by_percent) under the official £100.0m budget cap and 3-per-club limit.
    """
    cache_dir = os.path.join(base_dir, ".fpl_cache")
    sim_path = os.path.join(cache_dir, "simulations_10k_fixtures_1.json")
    sim_5_path = os.path.join(cache_dir, "simulations_10k_fixtures_5.json")
    history_path = os.path.join(cache_dir, "player_histories.json")
    ...
    if os.path.exists(sim_5_path):
        try:
            with open(sim_5_path, "r", encoding="utf-8") as f:
                d5 = json.load(f)
                players_5_map = {p["id"]: p for p in d5.get("players", [])}
        except Exception:
            players_5_map = {}
    ...
    cap_p = xi.get("captain") or starters_raw[0]
    vc_p = xi.get("vice_captain") or (starters_raw[1] if len(starters_raw) > 1 else starters_raw[0])
    ...
    starting_xi_ownership = round(sum(p.get("selected_by_percent", 0.0) for p in starters), 1)
    bench_ownership = round(sum(p.get("selected_by_percent", 0.0) for p in bench), 1)
    total_squad_ownership = round(starting_xi_ownership + bench_ownership, 1)

    first_fix_cap = cap_p.get("fixtures", [{}])[0]
    first_fix_vc = vc_p.get("fixtures", [{}])[0]
    opp_cap = first_fix_cap.get("opponent", "TBD")
    opp_vc = first_fix_vc.get("opponent", "TBD")
    ...
    captain_info = {
        ...
        "position": cap_p["position"],
        "price": cap_p["price"],
        "selected_by_percent": cap_p.get("selected_by_percent", 0.0),
        "xp": cap_xp,
        "doubled_xp": round(cap_xp * 2, 2),
        "opponent": opp_cap,
        "fdr": first_fix_cap.get("fdr", 3)
    ...
    vc_info = {
        ...
        "position": vc_p["position"],
        "price": vc_p["price"],
        "selected_by_percent": vc_p.get("selected_by_percent", 0.0),
        "xp": round(vc_p["xp"], 2),
        "opponent": opp_vc,
        "fdr": first_fix_vc.get("fdr", 3)
    }
    ...
    squad_id = f"gw{gw}_consensus_template_optimum"
    title = f"Consensus Template Squad (Most Owned Players): Gameweek {gw}"

    gkp_starter = formation_lines["gkp"][0] if formation_lines["gkp"] else {}
    gkp_backup = next((p for p in bench if p["position"] == "GKP"), {})

    notes = [
        f"Consensus FPL Template Squad solved to maximize cumulative ownership ({starting_xi_ownership:.1f}% XI, {total_squad_ownership:.1f}% total squad) within the £100.0m budget cap.",
        f"{xi['formation']} formation deploying the most popular Premier League assets while adhering strictly to club limits (max 3 per team).",
        f"Template captaincy assigned to {cap_p['name']} ({cap_p.get('selected_by_percent', 0.0):.1f}% ownership, {cap_p.get('xp', 0.0):.2f} xP) with {vc_p['name']} ({vc_p.get('selected_by_percent', 0.0):.1f}% ownership) as vice-captain.",
        f"Goalkeeper pairing features premium starter {gkp_starter.get('name', 'Raya')} ({gkp_starter.get('selected_by_percent', 0.0):.1f}% owned) supported by £4.0m reserve {gkp_backup.get('name', 'Dubravka')} ({gkp_backup.get('selected_by_percent', 0.0):.1f}% owned) to conserve funds.",
        f"Budget fully utilized: £{total_cost:.1f}m invested with £{bank_remaining:.1f}m in the bank (ITB)."
    ]
    ...
    return {
        ...
        "total_match_xp": total_match_xp,
        "full_squad_xp": full_squad_xp,
        "starting_xi_ownership": starting_xi_ownership,
        "total_squad_ownership": total_squad_ownership,
        "total_ownership": total_squad_ownership,
        "captain": captain_info,
        "vice_captain": vc_info,
        "formation_lines": formation_lines,
        "starters": starters,
...
def update_saved_squads_markdown(
    squads: Dict[Any, Dict[str, Any]],
    template_squad: Optional[Dict[str, Any]] = None,
    base_dir: str = "."
) -> None:
    """Updates SAVED_SQUADS.md with formatted tables and review notes for all horizons and template."""
    md_path = os.path.join(base_dir, "SAVED_SQUADS.md")
    today = time.strftime("%Y-%m-%d")

    if template_squad is None and "template" in squads:
        template_squad = squads["template"]
    ...
    sel_idx = 1
    horizon_keys = sorted([h for h in squads.keys() if isinstance(h, int)])
    for h in horizon_keys:
        ...
        if h == 1:
            ...
            for i, p in enumerate(s["bench"]):
                ...
                xp = p["xp"]
                avg = xp / h
                role = ("Rotating budget goalkeeper" if is_expected_starting_gkp(p) else "Non-playing budget keeper (frees funds for outfield talent)") if pos == "GKP" else f"{i}st Outfield Sub" if i == 1 else f"{i}nd Outfield Sub" if i == 2 else f"{i}rd Outfield Sub"
    ...
    if template_squad:
        ...
        tvc = t["vice_captain"]
        content.extend([
            f"## Selection {sel_idx}: Consensus Template Team (Most Owned Under £100.0m Budget)",
            "",
            f"- **Selection ID**: `{t['id']}`",
            f"- **Saved At**: {today}",
            f"- **Formation**: **{t['formation']}** *(Valid FPL formation)*",
            f"- **Budget**: £100.0m | **Cost**: £{t['total_cost']:.1f}m | **Bank**: £{t['bank_remaining']:.1f}m",
            f"- **Cumulative Ownership**: **{t.get('starting_xi_ownership', 0):.1f}% Starting XI** | **{t.get('total_squad_ownership', 0):.1f}% Full Squad**",
            f"- **Projected Starting XI xP**: **{t['starting_xi_xp']:.2f} pts**",
            f"- **Captain (2x)**: **{tc['name']}** ({tc.get('selected_by_percent', 0.0):.1f}% owned, +{tc['xp']:.2f} pts) $\\to$ **Total Match Projection: {t['total_match_xp']:.2f} pts**",
            f"- **Vice-Captain**: **{tvc['name']}** ({tvc.get('selected_by_percent', 0.0):.1f}% owned, {tvc['xp']:.2f} xP)",
            f"- **Full Squad xP**: **{t['full_squad_xp']:.2f} pts**",
            "",
            "### Starting XI Lineup",
        ...
        for p in t["starters"]:
            ...
            club = p["team"]
            price = p["price"]
            own = f"{p.get('selected_by_percent', 0.0):.1f}%"
            xp = p["xp"]
            floor = p.get("floor", "-")
            ceil = p.get("ceiling", "-")
            opp = p["fixtures"][0]["opponent"] if p.get("fixtures") else "-"
            content.append(f"| **{pos}** | {name} | {club} | £{price:.1f}m | {own} | {xp:.2f} | {floor} | {ceil} | *(pending)* | vs {opp} |")
        ...
        for i, p in enumerate(t["bench"]):
            ...
            own = f"{p.get('selected_by_percent', 0.0):.1f}%"
            xp = p["xp"]
            role = "Budget template backup keeper" if pos == "GKP" else f"{i}st Outfield Sub" if i == 1 else f"{i}nd Outfield Sub" if i == 2 else f"{i}rd Outfield Sub"
            content.append(f"| **{order}** | {pos} | {name} | {club} | £{price:.1f}m | {own} | {xp:.2f} | *(pending)* | {role} |")
...
def regenerate_all_optimal_squads(
    ...
    generates the consensus template squad, saves them to .fpl_cache and frontend/.fpl_cache,
    updates saved_squads.json, and updates SAVED_SQUADS.md.
    """
    ...
    generated_squads: Dict[Any, Dict[str, Any]] = {}
    for h in horizons:
    ...
    if include_template:
        print(f"\n[*] Solving consensus template squad (maximum ownership within £100.0m budget)...")
        template_squad = generate_template_squad(base_dir=base_dir)
        generated_squads["template"] = template_squad

        template_out = os.path.join(cache_dir, "optimal_squad_template.json")
        with open(template_out, "w", encoding="utf-8") as f:
            json.dump(template_squad, f, indent=2)
        print(f"  [✔] Saved {template_out} (Formation: {template_squad['formation']}, Ownership: {template_squad['starting_xi_ownership']}% XI / {template_squad['total_squad_ownership']}% Squad, Match xP: {template_squad['total_match_xp']} pts)")

        if os.path.exists(frontend_cache_dir):
            frontend_out = os.path.join(frontend_cache_dir, "optimal_squad_template.json")
            shutil.copy2(template_out, frontend_out)
            print(f"  [✔] Synced to {frontend_out}")
    ...
    for k in sorted(generated_squads.keys(), key=lambda x: (isinstance(x, str), x)):
        updated_squads.append(generated_squads[k])
